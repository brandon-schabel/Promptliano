...
export function createTicketService(deps?: TicketServiceDeps) {
  ...
  const extensions = {
    async createWithTasks(data: CreateTicket & { generateTasks?: boolean }): Promise<TicketWithTasks> {
    ...
    async bulkUpdateStatus(updates: Array<{ id: number; status: TicketStatus }>): Promise<number> {
      return withErrorContext(
        async () => {
  ...
  return composeServices({
    ...
    async createProjectWithStructure(data: CreateProjectWithStructure) {
      return withTransaction(async (tx) => {
        ...
        if (data.tickets) {
          await services.tickets.batch.create(data.tickets.map((t) => ({ ...t, projectId: project.id })))
        }
...
describe('TicketService', () => {
  ...
  test('should handle not found errors consistently', async () => {
    ...
    await expect(service.getById(999)).rejects.toThrow(ErrorFactory.notFound('Ticket', 999))
  })
})
```
...
export function createTicketService(deps?: TicketServiceDeps) {
  ...
  return extendService(base, {
    async createWithTasks(data: CreateTicket & { generateTasks?: boolean }) {
      ...
      const tasks = data.generateTasks ? await deps?.aiService?.generateTasks(ticket) : []
      return { ticket, tasks }
    }
  })
}
