...
async function main() {
  ...
  console.log("getProjectFileTree sample root children count:", Array.isArray(tree?.children) ? tree.children.length : 0);

  // Validate search through file-search-service
  const searchService = createFileSearchService();
  const { results, stats } = await searchService.search(projectId, { query: "import", limit: 5, searchType: "semantic" });
  console.log(`Search stats:`, stats);
  console.log(`Search results sample:`, results.slice(0, 2).map(r => ({ file: r.file.path, score: r.score, matches: r.matches?.length })));
