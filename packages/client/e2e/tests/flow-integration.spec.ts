...
test.describe('Flow Integration - UI Testing', () => {
  ...
  test.describe('Queue Overview View', () => {
    test.beforeEach(async () => {
      await appPage.page
        .locator('[data-testid="flow-sidebar"], .flow-sidebar')
        .locator('button:has-text("Queues"), button:has-text("Task Queues")')
        .click()
      await appPage.waitForLoadingComplete()
    })
    ...
    test('should display queue interface', async () => {
      ...
      const queueElements = appPage.page.locator(
        '[data-testid="queue-grid"], .queue-grid, [data-testid="queue-overview"], .queue-overview, text=/queue/i'
      )
    ...
    test('should show create queue option', async () => {
      const createQueueButton = appPage.page.locator(
        '[data-testid="create-queue"], button:has-text("Create Queue"), button:has-text("New Queue")'
      )
  ...
  test.describe('Tickets View', () => {
    test.beforeEach(async () => {
      await appPage.page
        .locator('[data-testid="flow-sidebar"], .flow-sidebar')
        .locator('button:has-text("Tickets")')
        .click()
      await appPage.waitForLoadingComplete()
    })
    ...
    test('should display tickets interface', async () => {
      ...
      const ticketsElements = appPage.page.locator(
        '[data-testid="tickets-view"], .tickets-view, [data-testid="ticket-list"], .ticket-list'
      )
    ...
    test('should show create ticket option', async () => {
      const createTicketButton = appPage.page.locator(
        '[data-testid="new-ticket"], button:has-text("New Ticket"), button:has-text("Create Ticket")'
      )
  ...
  test.describe('Kanban View', () => {
    test.beforeEach(async () => {
      await appPage.page
        .locator('[data-testid="flow-sidebar"], .flow-sidebar')
        .locator('button:has-text("Kanban")')
        .click()
      await appPage.waitForLoadingComplete()
    })
  ...
  test.describe('Analytics View', () => {
    ...
    test('should display analytics interface', async () => {
      ...
      const analyticsElements = appPage.page.locator(
        '[data-testid="analytics-view"], .analytics-view, text="Analytics"'
      )
