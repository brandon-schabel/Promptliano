...
export class FlakyTestPrevention {
   ...
   */
  static async waitForStableElement(locator: Locator, description: string, options: TimingOptions = {}): Promise<void> {
  ...
  static async waitForTableData(
    ...
    await expect(async () => {
      ...
      if (skipEmpty) {
        ...
        if (
          !hasContent ||
          hasContent.trim().length === 0 ||
          hasContent.includes('loading') ||
          hasContent.includes('No data')
        ) {
  ...
  static async interactWithModal(
    ...
    const { trigger, modalSelector = '[role="dialog"], .modal, [data-modal]', buttonText, timeout = 10000 } = options
  ...
  static async uploadFile(
    ...
    if (waitForUpload) {
      ...
      try {
        await Promise.race([
          page.waitForSelector('.upload-success, [data-upload-status="success"]', { timeout: uploadTimeout }),
          page.waitForSelector('.upload-complete, [data-upload="complete"]', { timeout: uploadTimeout }),
   ...
   */
  static async waitForAnimations(page: Page, locator?: Locator, timeout: number = 2000): Promise<void> {
    try {
      if (locator) {
