...
function ResearchDetailPage() {
  ...
  const handleTabChange = (tab: string) => {
    const validTabs = ['overview', 'sources', 'sections', 'document', 'debug'] as const
    ...
    <div className='max-w-7xl mx-auto p-6 space-y-6'>
      ...
      <div className='space-y-4'>
            ...
            {isEditing ? (
              <div className='space-y-3'>
                ...
                <div className='flex gap-2'>
                  <Button size='sm' onClick={handleSaveEdit} disabled={!editedTopic.trim() || updateResearch.isPending}>
                  ...
                  <Button size='sm' variant='outline' onClick={handleCancelEdit} disabled={updateResearch.isPending}>
              ...
              <>
                ...
                {researchData.description && <p className='text-muted-foreground mt-2'>{researchData.description}</p>}
              </>
            )}
          </div>
          <div className='flex items-center gap-2'>
            ...
            {workflowStatus?.data?.canExecute && (
              <Button onClick={handleExecuteWorkflow} disabled={executeWorkflow.isPending} variant='default' size='sm'>
            ...
            {workflowStatus?.data?.canResume && researchData.status === 'failed' && (
              <Button onClick={handleResumeWorkflow} disabled={resumeWorkflow.isPending} variant='default' size='sm'>
            ...
            {workflowStatus?.data?.canStop && (
              <Button onClick={handleStopWorkflow} disabled={stopWorkflow.isPending} variant='outline' size='sm'>
            ...
            <Badge variant={researchData.status === 'complete' ? 'default' : 'secondary'}>{researchData.status}</Badge>
          </div>
        </div>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
          ...
          {crawlProgress?.data?.crawlEnabled && <CrawlProgressDisplay researchId={numericResearchId} />}
        </div>
      </div>
      ...
      <Tabs value={currentTab} onValueChange={handleTabChange}>
        <TabsList>
          ...
          {crawlProgress?.data?.crawlEnabled && <TabsTrigger value='debug'>Debug</TabsTrigger>}
        </TabsList>
        ...
        <TabsContent value='sources'>
          <div className='space-y-6'>
            <div className='flex items-center justify-between'>
              ...
              {sources?.data && sources.data.length > 0 && (
                <Button
                  ...
                  disabled={
                    generateOutline.isPending || sources.data.some((s: ResearchSource) => s.status !== 'complete')
                  }
                  variant='default'
                >
            ...
            <div className='flex gap-2 p-4 border rounded-lg bg-muted/30'>
              ...
              <Button onClick={handleAddSource} disabled={!newSourceUrl.trim() || addSource.isPending}>
                {addSource.isPending ? <Loader2 className='h-4 w-4 animate-spin' /> : <Plus className='h-4 w-4' />}
              </Button>
            </div>
                  ...
                  <div key={source.id} className='p-4 border rounded-lg'>
                    <div className='flex items-start justify-between gap-4'>
                      <div className='flex-1 min-w-0'>
                        ...
                        {source.errorMessage && (
                          <div className='text-sm text-destructive mt-2'>Error: {source.errorMessage}</div>
                        )}
                      </div>
                      <div className='flex gap-2'>
                        <Button
                          ...
                          onClick={() => {
                            navigate({
                              ...
                              params: { researchId, sourceId: source.id.toString() },
                              search: { tab: 'overview' }
                            })
                          }}
                        >
                        ...
                        {source.status === 'complete' && (
                          <Button size='sm' variant='outline' onClick={() => handleViewSourceDetails(source.id)}>
                            <Eye className='h-4 w-4 mr-1' />
                            Details
                          </Button>
                        )}
        ...
        <TabsContent value='document'>
          <div className='space-y-4'>
            ...
            <div className='prose max-w-none p-6 border rounded-lg'>
              <p className='text-muted-foreground'>Document preview will be available when research is complete</p>
            </div>
          </div>
        </TabsContent>
      ...
      <SourceDetailsModal sourceId={selectedSourceId} isOpen={isSourceModalOpen} onClose={handleCloseSourceModal} />
    </div>
  )
}
