...
export function TicketListPanel({ projectTabId, onSelectTicket, onCreateTicket }: TicketListPanelProps) {
  ...
  const setTicketSort = useCallback(
    (val: string) => {
            ...
            return (
              ...
              >
                ...
                <div className='mt-2 mb-2'>
                  <Progress value={percentage} variant='fullness' className='h-1' />
                </div>
                <div className='mt-2 flex items-center justify-between'>
                  <div className='flex items-center gap-2'>
                    <Badge
                      className={cn(
                        PRIORITY_COLORS[ticket.ticket.priority as keyof typeof PRIORITY_COLORS] ||
                  ...
                  <div className='flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity'>
                    <Button variant='ghost' size='sm' onClick={(e) => copyOverview(e, ticket.ticket.overview ?? '')}>
                      <Copy className='h-4 w-4 mr-1' />
                      Copy Overview
                    </Button>
                    <Button variant='ghost' size='sm' onClick={(e) => handleCopyAll(e, ticket)}>
      ...
      <AlertDialog open={!!ticketToDelete} onOpenChange={(open) => !open && setTicketToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the ticket "{ticketToDelete?.ticket.title}" and
              all its associated tasks.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
