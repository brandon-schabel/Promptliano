...
export function createQueryKeys(domain: string): QueryKeyFactory {
  return {
    ...
    related: (id: number | string, relation: string) => [...[domain], 'detail', id, 'related', relation] as const,

    // Field-based queries
    byField: (field: string, value: any) => [...[domain], 'by', field, value] as const
  }
}
...
export function createExtendedQueryKeys(domain: string): ExtendedQueryKeyFactory {
  ...
  return {
    ...
    export: (format?: string) =>
      format ? ([...[domain], 'export', format] as const) : ([...[domain], 'export'] as const),
    import: (format?: string) =>
      format ? ([...[domain], 'import', format] as const) : ([...[domain], 'import'] as const),
    ...
    subscription: (channel?: string) =>
      channel ? ([...[domain], 'subscription', channel] as const) : ([...[domain], 'subscription'] as const),
...
export function createCompositeQueryKeys(): CompositeQueryKeyFactory {
  return {
    ...
    between: (domain1: string, id1: number | string, domain2: string) => ['composite', domain1, id1, domain2] as const,

    // Intersection of multiple domains
    intersection: (domains: string[]) => ['composite', 'intersection', ...domains] as const,

    // Union of multiple domains
    union: (domains: string[]) => ['composite', 'union', ...domains] as const,

    // Aggregate across domains
    aggregate: (domains: string[], metric: string) => ['composite', 'aggregate', metric, ...domains] as const
  }
}
