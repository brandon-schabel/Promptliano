...
export interface CompositeHooks<TPrimary> {
  ...
  useAggregation: (aggregationKey: string, params?: any) => UseQueryResult<any, Error>
...
export function createCompositeHooks<TPrimary extends { id: number | string }>(
   ...
   * Cross-domain query
   */
  const useCrossDomain = (params?: any, domains: string[] = crossDomain?.domains || []) => {
    const client = useApiClient()
   ...
   * Aggregation hook
   */
  const useAggregation = (aggregationKey: string, params?: any) => {
    const client = useApiClient()
   ...
   * Parent-child operations
   */
  const useParentChild = (parentId: number | string, childEntity: string) => {
    const client = useApiClient()
    const queryClient = useQueryClient()
   ...
   * Many-to-many associations
   */
  const useManyToMany = <TRelated = any>(entityId: number | string, relatedEntity: string) => {
    const client = useApiClient()
    const queryClient = useQueryClient()
  ...
  const useCascade = () => {
    ...
    const cascadeCreate = useCallback(
      async (data: any, relations?: Record<string, any[]>) => {
        if (!client) throw new Error('API client not initialized')
    ...
    const cascadeUpdate = useCallback(
      async (id: number | string, data: any, relations?: Record<string, any>) => {
        if (!client) throw new Error('API client not initialized')
  ...
  const useBulkRelations = () => {
    ...
    const attachMany = useCallback(
      async (parentId: number | string, childIds: (number | string)[], relation: string) => {
        if (!client) throw new Error('API client not initialized')
    ...
    const syncRelations = useCallback(
      async (parentId: number | string, relations: Record<string, (number | string)[]>) => {
        if (!client) throw new Error('API client not initialized')
   ...
   */
  const useGraph = (rootId: number | string, depth: number = 2) => {
    const client = useApiClient()
    ...
    const { data: graph, isLoading } = useQuery({
      ...
      queryFn: async () => {
        ...
        const traverse = async (entityName: string, id: number | string, level: number) => {
          ...
          if (level < depth) {
            const entityRelationships = entityName === primary.entityName ? relationships : {} // Would need to define relationships for other entities
