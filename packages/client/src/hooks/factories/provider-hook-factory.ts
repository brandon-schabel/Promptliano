...
export interface ProviderHookFactory<T extends APIProviders> {
  ...
  useProviderModels: (options?: { enabled?: boolean; staleTime?: number }) => {
...
export function createProviderHookFactory<T extends APIProviders>(
  ...
  return {
    ...
    useModelSelection: (defaultModel) => {
      ...
      const [selectedModel, setSelectedModel] = useState<ModelNamePattern<T> | null>(defaultModel || null)

      return useMemo(
        (): ModelSelectionHookConstraints<T> => ({
          provider,
...
class TypeSafeProviderRegistry {
  ...
  register<T extends APIProviders>(provider: T, config: ProviderConfig<T>): ProviderHookFactory<T> {
  ...
  get<T extends APIProviders>(provider: T): ProviderHookFactory<T> | undefined {
