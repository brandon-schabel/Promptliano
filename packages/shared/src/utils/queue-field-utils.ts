 ...
 * ```
 */
export function getProcessingDuration(item: Pick<QueueFields, 'queueStartedAt' | 'queueCompletedAt'>): number | null {
 ...
 * ```
 */
export function createCompleteProcessingUpdate(success: boolean, errorMessage?: string): QueueFieldUpdates {
  const now = Date.now()
