...
describe('schema-factories', () => {
  describe('createApiResponseSchema', () => {
    ...
    test('enforces success literal true', () => {
      ...
      expect(() => responseSchema.parse({ success: false, data: { id: 1 } })).toThrow()
    })
    ...
    test('validates data against provided schema', () => {
      ...
      expect(() => responseSchema.parse({ success: true, data: { id: -1 } })).toThrow()

      expect(() => responseSchema.parse({ success: true, data: { id: 'invalid' } })).toThrow()
    })
  ...
  describe('createFlexibleField', () => {
    test('preprocesses values before validation', () => {
      const field = createFlexibleField(z.number(), (value) =>
    ...
    test('runs validation after preprocessing', () => {
      const field = createFlexibleField(z.number().positive(), (value) =>
        typeof value === 'string' ? parseInt(value, 10) : (value as number)
      )
  ...
  describe('integration and real-world scenarios', () => {
    ...
    test('complex nested schemas', () => {
      ...
      const userData = {
        name: 'John Doe',
        email: 'john@example.com',
        addresses: [{ street: '123 Main St', city: 'Boston', zipCode: '02101' }],
        primaryAddress: null,
