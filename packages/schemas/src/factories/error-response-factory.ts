 ...
 */
export function createBatchErrorResponseSchema<T extends z.ZodTypeAny>(itemSchema: T, name?: string) {
 ...
 */
export function createPartialFailureResponseSchema<T extends z.ZodTypeAny>(itemSchema: T, name?: string) {
  const schemaName = name || getZodDescription(itemSchema) || 'Item'
    ...
    .object({
      ...
      error: z.object({
        ...
        retryAfter: options?.retryAfter
          ? z.number().default(options.retryAfter).describe('Seconds to wait before retry')
          : z.number().optional().describe('Seconds to wait before retry'),
        maxRetries: options?.maxRetries
          ? z.number().default(options.maxRetries).describe('Maximum retry attempts')
          : z.number().optional().describe('Maximum retry attempts'),
 ...
 */
export function createActionableErrorResponseSchema(code: string, message: string, actions: string[]) {
  return z
    .object({
      success: z.literal(false),
      error: z.object({
