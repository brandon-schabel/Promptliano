...
describe('Ticket Service (Isolated Database)', () => {
    ...
    describe('Basic CRUD Operations', () => {
        ...
        test('should throw error when updating non-existent ticket', async () => {
            // Act & Assert
            await expect(ticketService.update(99999, { suggestedFileIds: ['test-file'] })).rejects.toThrow('Failed to update Ticket with ID 99999')
        })
    ...
    describe('Edge Cases and Error Handling', () => {
        ...
        test('should handle concurrent operations', async () => {
            ...
            const promises = Array(5).fill(null).map(() =>
                ticketService.create(TestDataFactory.ticket(testProject.id))
            )
