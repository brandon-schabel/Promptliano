...
describe('SyncProgressTracker', () => {
  ...
  describe('event ordering', () => {
    test('should assign sequential sequence numbers', async () => {
      ...
      const sequenceNumbers = progressEvents.filter((e) => e.sequenceNumber !== undefined).map((e) => e.sequenceNumber!)
  ...
  describe('reset', () => {
    test('should reset all state correctly', async () => {
      ...
      const eventsAfterReset = progressEvents.filter((e) => e.phase === 'initializing' && e.processedFiles === 0)
