...
describe('file-importance-scorer', () => {
  describe('getFileImportance', () => {
    ...
    describe('location scoring', () => {
      ...
      test('scores test directories low', () => {
        ...
        expect(getFileImportance(test1).factors.location).toBeLessThan(getFileImportance(srcFile).factors.location)
      ...
      test('penalizes deep nesting', () => {
        ...
        expect(getFileImportance(shallow).factors.location).toBeGreaterThan(getFileImportance(deep).factors.location)
      })
  ...
  describe('sortFilesByImportance', () => {
    ...
    test('preserves original array', () => {
      const files = [createTestFile({ id: 1 }), createTestFile({ id: 2 })]
  ...
  describe('filterByImportance', () => {
    ...
    test('returns empty array when no files meet threshold', () => {
      const files = [createTestFile({ name: 'test.txt', size: 10 })]
