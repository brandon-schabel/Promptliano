...
export function createServiceContainer(config: ServiceContainerConfig = {}): ServiceContainer {
  ...
  const domainServices = {
     ...
     */
    async createProjectWithStructure(data: { project: any; tickets?: any[]; initialFiles?: any[]; prompts?: any[] }) {
      ...
      const results = await Promise.allSettled([
        ...
        tickets.length > 0
          ? Promise.all(tickets.map((ticket) => ticketService.create({ ...ticket, projectId: project.id })))
          : Promise.resolve([]),
