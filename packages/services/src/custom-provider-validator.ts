...
export async function validateCustomProvider(
  ...
  if (!urlValidation.valid) {
    throw new ApiError(400, urlValidation.error || 'Invalid provider URL', 'INVALID_PROVIDER_URL')
  }
  ...
  const normalizedUrl = sanitizedUrl.endsWith('/v1') ? sanitizedUrl : `${sanitizedUrl.replace(/\/$/, '')}/v1`
...
async function detectProviderFeatures(
  ...
  features.embeddings = models.some(
    (m) => m.id.toLowerCase().includes('embed') || m.name.toLowerCase().includes('embed')
  )

  // Check for vision models (usually contain 'vision' or 'gpt-4-vision' in the name)
  features.vision = models.some(
    (m) => m.id.toLowerCase().includes('vision') || m.id.toLowerCase().includes('gpt-4-turbo')
  )
