...
export const fileRepository = {
  ...
  async getByProject(projectId: number, options?: { limit?: number; offset?: number }): Promise<File[]> {
    const query = db.select().from(files).where(eq(files.projectId, projectId))
  ...
  async getByPaths(projectId: number, paths: string[]): Promise<File[]> {
    ...
    return db
      .select()
      .from(files)
      .where(and(eq(files.projectId, projectId), inArray(files.path, paths)))
  },
  ...
  async update(id: string, data: Partial<Omit<InsertFile, 'createdAt' | 'updatedAt'>>): Promise<File> {
    ...
    const [updated] = await db.update(files).set(updateData).where(eq(files.id, id)).returning()
  ...
  async getSelectedFiles(projectId: number): Promise<SelectedFile[]> {
    return db.select().from(selectedFiles).where(eq(selectedFiles.projectId, projectId))
  },
  ...
  async deselectFile(projectId: number, fileId: string): Promise<boolean> {
    const result = (await db
      ...
      .where(and(eq(selectedFiles.projectId, projectId), eq(selectedFiles.fileId, fileId)))
  ...
  async clearSelectedFiles(projectId: number): Promise<number> {
    const result = (await db.delete(selectedFiles).where(eq(selectedFiles.projectId, projectId)).run()) as unknown as {
      changes: number
    }
