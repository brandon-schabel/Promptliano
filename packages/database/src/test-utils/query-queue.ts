...
/**
 * Create a proxy for Drizzle query builders to ensure method chaining works with queuing
 */
function createProxiedQueryBuilder<T>(originalBuilder: T, queue: QueryQueue, operationPrefix: string): T {
  return new Proxy(originalBuilder as any, {
    get(target, prop: string | symbol) {
      const value = target[prop]

      if (typeof value === 'function' && typeof prop === 'string') {
        return (...args: any[]) => {
          const operation = `${operationPrefix}.${prop}(${args.length} args)`

          // For terminal methods that execute queries, queue them
          if (['run', 'get', 'all', 'values', 'returning', 'execute'].includes(prop)) {
            return queue.execute(() => {
              return value.apply(target, args)
            }, operation)
          }

          // For chaining methods, apply synchronously but return proxied result
          const result = value.apply(target, args)
          return createProxiedQueryBuilder(result, queue, operation)
        }
      }
    ...
    get(target, prop: string | symbol) {
      ...
      if (typeof value === 'function' && typeof prop === 'string') {
        ...
        return (...args: any[]) => {
          ...
          // For Drizzle ORM query builder methods, return proxied builder
          if (['insert', 'update', 'delete', 'select'].includes(prop)) {
            const result = value.apply(target, args)
            return createProxiedQueryBuilder(result, queue, prop)
          }
