...
export function validateQuery<T>(schema: z.ZodSchema<T>, options: ValidationOptions = {}) {
  return async (c: Context, next: () => Promise<void>) => {
    try {
      ...
      const parsed = options.coerce !== false ? coerceQueryParams(query, schema) : query
...
export function validateParams<T>(schema: z.ZodSchema<T>, options: ValidationOptions = {}) {
  return async (c: Context, next: () => Promise<void>) => {
    try {
      ...
      const parsed = options.coerce !== false ? coerceParams(params, schema) : params
 ...
 */
export function validateRequest<TBody = any, TQuery = any, TParams = any, THeaders = any>(
  schemas: {
  ...
  return async (c: Context, next: () => Promise<void>) => {
    ...
    if (schemas.query) {
      try {
        const query = c.req.query()
        const parsed = options.coerce !== false ? coerceQueryParams(query, schemas.query) : query
    ...
    if (schemas.params) {
      try {
        const params = c.req.param()
        const parsed = options.coerce !== false ? coerceParams(params, schemas.params) : params
 ...
 * Custom validation rules
 */
export function validateCustom(validator: (c: Context) => Promise<void> | void) {
  return async (c: Context, next: () => Promise<void>) => {
    await validator(c)
    await next()
  }
...
function coerceQueryParams(query: Record<string, string | string[]>, schema: z.ZodSchema): any {
  ...
  Object.keys(query).forEach((key) => {
    ...
    if (typeName === 'ZodNumber' || fieldSchema._def?.coerce === 'number') {
      result[key] = Array.isArray(value) ? value.map((v) => parseFloat(v)) : parseFloat(value as string)
 ...
 */
export function conditionalValidation<T>(condition: (c: Context) => boolean, schema: z.ZodSchema<T>) {
  return async (c: Context, next: () => Promise<void>) => {
    if (condition(c)) {
...
export function validateArrayLength(field: string, min?: number, max?: number) {
  return async (c: Context, next: () => Promise<void>) => {
    ...
    if (min !== undefined && array.length < min) {
      throw ErrorFactory.invalidInput(field, `array with at least ${min} items`, `array with ${array.length} items`)
    }

    if (max !== undefined && array.length > max) {
      throw ErrorFactory.invalidInput(field, `array with at most ${max} items`, `array with ${array.length} items`)
    }
