...
describe('Flow Integration Tests', () => {
    ...
    test('should handle complete project workflow from creation to completion', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          const queueStats = await tracker.measure('check-queue-stats', () => client.queues.getQueueStats(queue.id))
          ...
          console.log(
            `✅ Queue stats verified: ${queueStats.data.totalItems} total items, ${queueStats.data.queuedItems} pending`
          )
          ...
          console.log(
            `✅ Final stats: ${finalStats.data.completedItems} completed, ${finalStats.data.queuedItems} pending`
          )
    ...
    test('should handle concurrent workflow operations', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          console.log(
            `✅ Concurrent processing completed: ${totalProcessed} items processed across ${queues.length} queues`
          )
        })
      })
    })
  ...
  describe('AI-Enhanced Workflow Integration', () => {
    test.skipIf(!process.env.AI_TEST_MODE)(
      'should create complete AI-assisted workflow',
      async () => {
        await withTestEnvironment(async (env: TestEnvironment) => {
          ...
          await withTestData(env, async (dataManager: TestDataManager) => {
            ...
            const ticket = await dataManager.createTicket(
              factories.createTicketData({
                ...
                description:
                  'Create a complete authentication system with JWT tokens, password hashing, email verification, and role-based access control'
              })
            )
            ...
            const aiTasks = await retryOperation(() => client.tickets.autoGenerateTasks(ticket.id), {
              maxRetries: 2,
              delay: 3000
            })
            ...
            const queue = await dataManager.createQueue(
              project.id,
              factories.createQueueData({
    ...
    test.skipIf(!process.env.AI_TEST_MODE)(
      'should handle AI task suggestions in workflow',
      async () => {
        await withTestEnvironment(async (env: TestEnvironment) => {
          const lmStudioCheck = await checkLMStudioAvailability(env.config.ai.lmstudio)
          if (!lmStudioCheck.available) {
          ...
          await withTestData(env, async (dataManager: TestDataManager) => {
            ...
            const ticket = await dataManager.createTicket(
              factories.createTicketData({
                ...
                description:
                  'Build a real-time chat system with websockets, message persistence, and user presence indicators'
              })
            )
    ...
    test('should handle large-scale workflow operations', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          const allTasks = await tracker.measure('create-all-tasks', () => Promise.all(taskPromises))
          ...
          await tracker.measure('distribute-across-queues', () => Promise.all(distributionPromises))

          // Verify distribution
          const allStats = await Promise.all(queues.map((queue) => client.queues.getQueueStats(queue.id)))
          ...
          // Verify final state
          const finalStats = await Promise.all(queues.map((queue) => client.queues.getQueueStats(queue.id)))
