...
describe('Tickets API Tests', () => {
    ...
    test('should handle ticket status filtering', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          const openTicket = await dataManager.createTicket(
          ...
          const inProgressTicket = await dataManager.createTicket(
            factories.createTicketData({
              projectId: project.id,
  ...
  describe('AI-Powered Features', () => {
    test.skipIf(!process.env.AI_TEST_MODE)(
      'should suggest tasks for tickets',
      async () => {
        await withTestEnvironment(async (env: TestEnvironment) => {
          ...
          await withTestData(env, async (dataManager: TestDataManager) => {
            ...
            const ticket = await dataManager.createTicket(
              factories.createTicketData({
                ...
                description:
                  'Create a complete authentication system with login, registration, and password reset functionality'
              })
            )
            const client = createPromptlianoClient({ baseUrl: env.baseUrl })
    ...
    test.skipIf(!process.env.AI_TEST_MODE)(
      'should auto-generate tasks for tickets',
      async () => {
        await withTestEnvironment(async (env: TestEnvironment) => {
          const lmStudioCheck = await checkLMStudioAvailability(env.config.ai.lmstudio)
          if (!lmStudioCheck.available) {
          ...
          await withTestData(env, async (dataManager: TestDataManager) => {
            ...
            const generateResult = await retryOperation(() => client.tickets.autoGenerateTasks(ticket.id), {
              maxRetries: 2,
              delay: 3000,
    ...
    test.skipIf(!process.env.AI_TEST_MODE)(
      'should suggest relevant files for tickets',
      async () => {
        await withTestEnvironment(async (env: TestEnvironment) => {
          const lmStudioCheck = await checkLMStudioAvailability(env.config.ai.lmstudio)
          if (!lmStudioCheck.available) {
