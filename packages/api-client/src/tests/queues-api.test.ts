  ...
  describe('Basic Queue CRUD Operations', () => {
    ...
    test('should handle queue error scenarios', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        ...
        try {
          await client.queues.createQueue(99999, {
    ...
    test('should enqueue tickets with all their tasks', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          const ticketItems = queueItems.data.filter(
            (item) => item.queueItem.itemType === 'ticket' && item.queueItem.itemId === tickets[0].id
          )
          expect(ticketItems.length).toBe(1)

          const taskItems = queueItems.data.filter(
            (item) =>
    ...
    test('should handle batch enqueue operations', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          const ticket1Item = queueItems.data.find(
            (item) => item.queueItem.itemType === 'ticket' && item.queueItem.itemId === tickets[0].id
          )
          expect(ticket1Item?.queueItem.priority).toBe(10)

          const ticket2Item = queueItems.data.find(
            (item) => item.queueItem.itemType === 'ticket' && item.queueItem.itemId === tickets[1].id
          )
    ...
    test('should identify unqueued items for a project', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          const ticket1 = await dataManager.createTicket(
            factories.createTicketData({ projectId: project.id, title: 'Queued Ticket' })
          )
          const ticket2 = await dataManager.createTicket(
            factories.createTicketData({ projectId: project.id, title: 'Unqueued Ticket' })
          )
    ...
    test('should handle high-volume queue operations', async () => {
      await withTestEnvironment(async (env: TestEnvironment) => {
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          const itemsResult = await tracker.measure('get-queue-items', () => client.queues.getQueueItems(queue.id))

          assertions.assertSuccessResponse(itemsResult)
          expect(itemsResult.data.length).toBe(ticketCount)

          // Test getting stats performance
          const statsResult = await tracker.measure('get-queue-stats', () => client.queues.getQueueStats(queue.id))
        ...
        await withTestData(env, async (dataManager: TestDataManager) => {
          ...
          try {
            await client.queues.batchEnqueue(queue.id, {
              items: [{ type: 'ticket', id: 99999, priority: 5 }]
            })
            expect(false).toBe(true) // Should not reach here
          } catch (error) {
            expect(error).toBeInstanceOf(Error)
