...
export function TanStackForm<T extends Record<string, any>>({
      ...
      onChange:
        validationMode === 'onChange'
          ? ({ value }) => {
              ...
              return result.success ? undefined : result.error.issues.map((err: z.ZodIssue) => err.message).join(', ')
            }
          : undefined,
      onBlur:
        validationMode === 'onBlur'
  ...
  const calculateProgress = () => {
    ...
    const completedFields = Object.values(form.state.values || {}).filter(
  ...
  const formState: FormState = {
    ...
    touchedFields: new Set(
      Object.keys(form.state.fieldMeta || {}).filter((key) => {
  ...
  const FormContent = (
    <div className={cn('w-full', className)}>
      ...
      <form
        ...
        className={cn(spacingClasses[layout.spacing || 'md'], 'w-full')}
      >
        {/* Form Fields */}
        <div className={cn('grid gap-4', gridClasses[layout.columns || 1])}>
          <TanStackFormProvider value={form}>{children}</TanStackFormProvider>
        </div>
          ...
          <>
            ...
            <div className='flex items-center justify-end gap-3'>
              ...
              <Button
                ...
                disabled={submitButton.disabled || !formState.isValid || formState.isSubmitting || isDisabled}
                className={cn('min-w-24', submitButton.className)}
              >
...
export function useTanStackFormState(form: any): FormState {
  return {
    ...
    submitCount: 0, // submitAttempts doesn't exist in TanStack Form
    errors: {},
    touchedFields: new Set(Object.keys(form.state.fieldMeta).filter((key) => form.state.fieldMeta[key]?.isTouched))
  }
}
