...
export function SidebarNav<T extends string = string>({
  ...
  const renderNavItem = (item: SidebarNavItem<T>) => {
    ...
    return (
      <Button
        ...
        disabled={isDisabled}
      >
        <Icon className={cn('shrink-0', itemVariant === 'compact' ? 'h-4 w-4' : 'h-4 w-4')} />
  ...
  const navContent = (
    <>
      {header && <div className={cn('mb-2', isHorizontal ? 'px-2' : 'px-2 pb-2 border-b')}>{header}</div>}

      <div className={cn('gap-1', isHorizontal ? 'flex flex-row px-2' : 'flex flex-col px-2 py-1')}>
        {items.map(renderNavItem)}
      </div>

      {footer && <div className={cn('mt-auto', isHorizontal ? 'px-2' : 'px-2 pt-2 border-t')}>{footer}</div>}
    </>
  )

  if (isHorizontal) {
    return <div className={cn('flex items-center', className)}>{navContent}</div>
  }

  return <div className={cn('flex flex-col h-full', className)}>{navContent}</div>
}
          ...
          {sections.map((section, index) => (
            <div key={index}>
              {showSectionTitles && section.title && (
                <div className='text-xs font-medium text-muted-foreground px-2 py-2 group-data-[collapsible=icon]:hidden'>
                  {section.title}
                </div>
              )}
              <div className={cn(
                ...
                {section.items.map((item) => {
                  const Icon = item.icon
                  const isActive =
                    (item as any).isActive !== undefined ? (item as any).isActive : activeItem === item.id
                  const isDisabled = item.enabled === false
                  const itemVariant = item.variant || variant
