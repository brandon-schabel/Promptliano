...
import { FormFactory, createFormComponent, type FormConfig } from '../forms/form-factory'
...
import { FileUploadInput, type FileUploadInputProps } from '../file/file-upload-input'
import {
  Search,
  Upload,
  Eye,
  ...
  const CreateModal = React.memo(
    ({
      ...
      return (
        ...
        >
          <FormFactory {...formConfig} onSubmit={handleSubmit} defaultValues={initialData} />
        </FormDialog>
      )
    }
  )
  ...
  const EditModal = React.memo(
    ({
      ...
      return (
        ...
        >
          <FormFactory {...formConfig} onSubmit={handleSubmit} defaultValues={item} />
        </FormDialog>
      )
    }
  )
    ...
    const handleSelect = (item: T) => {
      if (config.multiSelect) {
        ...
        const isSelected = selectedItems.some(
          (selected) => (config.keyExtractor ? config.keyExtractor(selected) : (selected as any).id) === key
        )
          ...
          <ScrollArea className='max-h-[400px] border rounded-md'>
            <div className='p-4 space-y-2'>
              {loading ? (
                ...
                <div className='text-center py-8 text-muted-foreground'>{config.emptyMessage || 'No items found'}</div>
              )}
            </div>
          </ScrollArea>
        </div>
...
export function createUploadModal(config: UploadModalConfig) {
  return React.memo(({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
    ...
    return (
      ...
      >
        <div className='space-y-4'>
          <FileUploadInput {...config.uploadProps} onFileSelect={handleFileChange} disabled={uploading} />
...
export { DialogBase, FormDialog, ConfirmationDialog } from '../overlay/dialog-base'

export type { DialogBaseProps, FormDialogProps, ConfirmationDialogProps, DialogAction } from '../overlay/dialog-base'
