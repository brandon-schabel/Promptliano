...
export function createEnhancedCrudModal<T extends { id?: number }>(options: EnhancedCrudModalOptions<T>) {
  const { schema, formConfig, fields, permissions = {}, confirmations = {}, ...baseConfig } = options
  ...
  return {
    ...
    EditModal: ({
      ...
      return (
        <crudModals.EditModal formConfig={editFormConfig!} isOpen={isOpen} onClose={onClose} item={item} {...props} />
      )
    },
    ...
    ViewModal: ({
      ...
      const renderContent = fields?.view
        ? (item: T) => (
      ...
      return (
        <crudModals.ViewModal isOpen={isOpen} onClose={onClose} item={item} renderContent={renderContent} {...props} />
      )
    }
  }
}
...
export function createModalSuite<T extends { id?: number }>(
  ...
  const searchModal = config.searchConfig
    ? createSearchModal<T>({
        ...config.searchConfig,
  ...
  const workflowModal = config.workflowConfig
    ? createWorkflowModal({
  ...
  const uploadModal = config.uploadConfig
    ? createUploadModal({
        ...config.uploadConfig,
  ...
  const renderModals = React.useCallback(() => {
    ...
    return (
      <>
        ...
        <modals.EditModal isOpen={state.isOpen && currentModal === 'edit'} onClose={closeAll} item={item} />

        <modals.DeleteModal isOpen={state.isOpen && currentModal === 'delete'} onClose={closeAll} item={item} />

        <modals.ViewModal isOpen={state.isOpen && currentModal === 'view'} onClose={closeAll} item={item} />
...
export const commonFormatters = {
  ...
  badge: (colorMap: Record<string, string>) => (value: string) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-medium ${colorMap[value] || 'bg-gray-100'}`}>{value}</span>
    )
  }
}
...
export const commonValidators = {
  ...
  unique:
    (checkUnique: (value: any) => Promise<boolean>, message = 'This value must be unique') =>
    async (value: any) => {
