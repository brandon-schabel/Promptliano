...
const statusIconVariants = cva('inline-flex items-center justify-center', {
  variants: {
    variant: {
      ...
      <div ref={ref} className={cn('inline-flex items-center gap-2', className)} {...props}>
        {showIcon ? (
          shouldShowSpinner && status === 'connecting' ? (
            ...
            <div className={statusIconVariants({ variant: computedVariant, size })}>{displayIcon}</div>
          )
        ) : (
          <div
            className={cn(
...
export const ConnectionStatus = React.forwardRef<HTMLDivElement, ConnectionStatusProps>(
  ({ isConnected, isConnecting = false, error, endpoint, latency, ...props }, ref) => {
    ...
    return <StatusIndicator ref={ref} status={status} details={details} showIcon {...props} />
  }
)
