...
export function ConfiguredDataTable<TData>({
  ...
  const [internalSorting, setInternalSorting] = React.useState<SortingState>(sorting.defaultSort || [])
  ...
  const pageCount =
    pagination.serverSide && pagination.totalCount
...
export function useDataTableState<TData>(config?: {
  ...
  const [sorting, setSorting] = React.useState<SortingState>(config?.defaultSorting || [])
  const [filters, setFilters] = React.useState<ColumnFiltersState>(config?.defaultFilters || [])
